...
def erode_edges(mask, erosion_width):
    ...
    if mask.ndim not in {2, 3}:
        raise ValueError('erode_edges expects arrays of ndim 2 or 3.Got ndim: {}'.format(mask.ndim))
...
def merge_preproc(label_list, x_list, y_list, shapes, overlap=100, type=np.uint8):
    if len(label_list) == 1:
        logger.info("label list length is one")
        return label_list[0].get()

    if not isinstance(label_list, list):
        logger.error("label list is not a list")
        return label_list
