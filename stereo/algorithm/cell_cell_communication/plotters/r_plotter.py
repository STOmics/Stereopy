...
from stereo.algorithm.cell_cell_communication.exceptions import MissingR, MissingPlotterFunctionException, RRuntimeException
from stereo.algorithm.cell_cell_communication.analysis_helper import get_separator
...
def heatmaps_plot(*,
    ...
    meta_file_separator = get_separator(os.path.splitext(meta_file)[-1])
    pvalues_file_separator = get_separator(os.path.splitext(pvalues_file)[-1])
    ...
    count_network_separator = get_separator(os.path.splitext(count_network_filename)[-1])
    interaction_count_separator = get_separator(os.path.splitext(interaction_count_filename)[-1])
