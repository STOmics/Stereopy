...
class PlotCellCellCommunication(PlotBase):
    ...
    def dot_plot(self,
        ...
        dot_plot = sns.scatterplot(data=result, x="cluster_pair", y="interacting_pair", palette=palette,
                                   hue='log2_mean', size='n_log10_p', sizes=(50, 250), legend='auto', **kwargs)
    ...
    def heatmap(self,
        ...
        heatmap_plot = sns.heatmap(data=network, square=True, cmap=palette, cbar_kws={'pad': 0.25, 'shrink': 0.5},
                                   **kwargs)
        ...
        plt.box(True)
        heatmap_plot = sns.heatmap(data=log_network, square=True, cmap=palette, cbar_kws={'pad': 0.25, 'shrink': 0.5},
                                   **kwargs)
        heatmap_plot.yaxis.set_ticks_position('right')
        heatmap_plot.invert_yaxis()
    ...
    def circos_plot(self,
                    significant_path: str = r'E:\算法组SVN\Personal\liuxiaobin\Stereopy\Fig5素材\significant_means_statistical_liana_EC.csv',
                    separator_cluster: str = '|',
                    cluster_pair: list = None,
                    output_path: str = r'E:\Stereopy\out',
                    output_name: str = r'circos_plot.pdf',
                    palette: str = "RdYlBu_r"
                    ):
        ...
        :param significant_path: path to the significant result
        :param separator_cluster: separator used for cell cluster pairs
        :param cluster_pair: if None, use all cluster pairs in the significant result; else list selected cluster pairs
        used in the plot
        ...
        for source in cell_types:
            for target in cell_types:
                if source + '|' + target in cluster_pair:
                    number_matrix.loc[source, target] = significant_df[source + '|' + target].count()
    ...
    def sankey_plot(self,
                    ...
                    weighted_network_path: str = r'E:\Stereopy\论文写作\Nichenet分析\weighted_network_lr_sig.txt',
                    regulon_path: str = r'E:\算法组SVN\Personal\liuxiaobin\Stereopy\Fig5素材\scoexp_regulons.json',
        ...
        :param meta: This dataframe must have a column named "cell_type" and a column named "cell"
        :param counts: normalized gene expressions, dataframe with genes as indexes, cells as columns
        ...
        significant result have been transferred to 'HUMAN', we need to transfer them back in order to match the gene
        names in counts.
        ...
        :param regulon_path: path to the spaGRN regulon output
        ...
        significant_separator = get_separator(os.path.splitext(significant_path)[-1])
        significant_df = pd.read_csv(significant_path, sep=significant_separator)
        significant_df = significant_df[['interacting_pair', sender_cluster + separator_cluster + receiver_cluster]]
        significant_df = significant_df.dropna(how='all',
                                               subset=significant_df.columns.drop('interacting_pair')).reset_index(drop=True)
...
if __name__ == "__main__":
    import scanpy as sc
    adata = sc.read_h5ad(r'E:\Stereopy\论文写作\数据\adata_VCM_EC.h5ad')
    sc.pp.normalize_total(adata)
    counts = pd.DataFrame(adata.X.T.toarray(), columns=adata.obs.index, index=adata.var.index)
    cell = list(adata.obs.index)
    cell_type = list(adata.obs['celltype'])
    meta = pd.DataFrame({"cell": cell, "cell_type": cell_type})
    ...
    # pltt.circos_plot(
    #     significant_path=r'E:\Stereopy\out\significant_means_statistical.csv',
    #     separator_cluster='|',
    #     cluster_pair=['Myeloid|Myeloid', 'Myeloid|NKcells_0', 'Myeloid|NKcells_1', 'Myeloid|Tcells',
    #                   'NKcells_0|Myeloid', 'NKcells_0|NKcells_0', 'NKcells_0|NKcells_1', 'NKcells_0|Tcells',
    #                   'NKcells_1|Myeloid'],
    #     output_path=r'E:\Stereopy\out',
    #     output_name=r'circos_plot.pdf')

    pltt.sankey_plot(meta=meta, counts=counts, sender_cluster='endocardial/endothelial (EC)',
                     receiver_cluster='ventricular-specific CM', homo_transfer=True)
