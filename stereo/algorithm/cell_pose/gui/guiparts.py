...
def create_channel_choose():
    # choose channel
    channel_choose = [QComboBox(), QComboBox()]
    channel_labels = []
    channel_choose[0].addItems(['gray', 'red', 'green', 'blue'])
    channel_choose[1].addItems(['none', 'red', 'green', 'blue'])
    cstr = ['chan to segment:', 'chan2 (optional): ']
    for i in range(2):
        channel_labels.append(QLabel(cstr[i]))
        if i == 0:
            channel_labels[i].setToolTip(
                'this is the channel in which the cytoplasm or nuclei exist that you want to segment')
            channel_choose[i].setToolTip(
                'this is the channel in which the cytoplasm or nuclei exist that you want to segment')
        else:
            channel_labels[i].setToolTip(
                'if <em>cytoplasm</em> model is chosen, and you also have a nuclear channel, then choose the nuclear'
                ' channel for this option')
            channel_choose[i].setToolTip(
                'if <em>cytoplasm</em> model is chosen, and you also have a nuclear channel, then choose the nuclear'
                ' channel for this option')

    return channel_choose, channel_labels
...
class ImageDraw(pg.ImageItem):
    ...
    **Bases:** :class:`GraphicsObject <pyqtgraph.GraphicsObject>`
    ...
    def is_at_start(self, pos):
        ...
        if not len(self.parent.current_stroke) > 3:
            return
