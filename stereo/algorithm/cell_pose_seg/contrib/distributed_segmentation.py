...
def segment(
    ...
    for index, input_block in block_iter:
        labeled_block, n = dask.delayed(segment_chunk, nout=2)(
            input_block, channels, model_type, diameter_yx, anisotropy, fast_mode, index
        )
    ...
    if np.prod(block_labeled.numblocks) > 1:
        ...
        block_labeled = da.overlap.trim_internal(block_labeled, trim_depth, boundary=boundary)
        block_labeled = link_labels(block_labeled, total, iou_depth, iou_threshold=iou_threshold)

        block_labeled = da.overlap.trim_internal(block_labeled, iou_depth, boundary=boundary)

    else:
        block_labeled = da.overlap.trim_internal(block_labeled, depth, boundary=boundary)
...
def segment_chunk(
    ...
    segments, _, _, _ = model.eval(
        chunk, channels=channels, z_axis=0, channel_axis=3, diameter=diameter_yx, do_3D=True, anisotropy=anisotropy,
        net_avg=not fast_mode, augment=not fast_mode, tile=not fast_mode
    )
...
def link_labels(block_labeled, total, depth, iou_threshold=1.0):
    """
