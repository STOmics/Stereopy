...
from stereo.constant import END_CELLPOSE
from stereo.constant import CELLPOSE_GUI_PNG_URL
from stereo.constant import STYLE_CHOICE_NPY_URL
from stereo.constant import CELLPOSE_TRANSPARENT_PNG_URL
...
def run():
    ...
    icon_path = pathlib.Path.home().joinpath(END_CELLPOSE, 'logo.png')
    guip_path = pathlib.Path.home().joinpath(END_CELLPOSE, 'cellpose_gui.png')
    style_path = pathlib.Path.home().joinpath(END_CELLPOSE, 'style_choice.npy')
    if not icon_path.is_file():
        cp_dir = pathlib.Path.home().joinpath(END_CELLPOSE)
        cp_dir.mkdir(exist_ok=True)
        print('downloading logo')
        download_url_to_file(CELLPOSE_TRANSPARENT_PNG_URL, icon_path, progress=True)
    if not guip_path.is_file():
        print('downloading help window image')
        download_url_to_file(CELLPOSE_GUI_PNG_URL, guip_path, progress=True)
    if not style_path.is_file():
        print('downloading style classifier')
        download_url_to_file(STYLE_CHOICE_NPY_URL, style_path, progress=True)
...
def get_unique_points(set):
    ...
    for k, pp in enumerate(set):
        cps[k, :] = np.array(pp)
    return list(np.unique(cps, axis=0))
...
class MainW(QMainWindow):
    ...
    def __init__(self, image=None):
        ...
        self.training_params = {'model_index': 0, 'learning_rate': 0.1, 'weight_decay': 0.0001, 'n_epochs': 100,
    ...
    def make_buttons(self):
        ...
        for i in range(2):
            ...
            if i == 0:
                ...
                label.setToolTip(
                    'if <em>cytoplasm</em> model is chosen, and you also have a nuclear channel, then choose the '
                    'nuclear channel for this option')
                self.ChannelChoose[i].setToolTip(
                    'if <em>cytoplasm</em> model is chosen, and you also have a nuclear channel, then choose the '
                    'nuclear channel for this option')
        ...
        label.setToolTip(
            'threshold on flow error to accept a mask (set higher to get more cells, e.g. in range from (0.1, 3.0),'
            ' OR set to 0.0 to turn off so no cells discarded);\n press enter to recompute if model already run')
        label.setStyleSheet(label_style)
        label.setFont(self.medfont)
        self.l0.addWidget(label, b, 0, 1, 5)
        ...
        label.setToolTip(
            'threshold on cellprob output to seed cell masks (set lower to include more pixels or higher to include'
            ' fewer, e.g. in range from (-6, 6)); \n press enter to recompute if model already run')
        label.setStyleSheet(label_style)
        label.setFont(self.medfont)
        self.l0.addWidget(label, b, 0, 1, 5)
        ...
        label.setToolTip(
            'for 3D volumes, turn on stitch_threshold to stitch masks across planes instead of running cellpose in'
            ' 3D (see docs for details)')
        label.setStyleSheet(label_style)
        label.setFont(self.medfont)
        self.l0.addWidget(label, b, 0, 1, 5)
    ...
    def keyPressEvent(self, event):
        if self.loaded:
            ...
            if not (event.modifiers() & (
                    QtCore.Qt.ControlModifier | QtCore.Qt.ShiftModifier | QtCore.Qt.AltModifier) or self.in_stroke):
                updated = False
    ...
    def undo_remove_cell(self):
        if not len(self.removed_cell) > 0:
            return
    ...
    def add_set(self):
        if not len(self.current_point_set) > 0:
            return
    ...
    def train_model(self):
        ...
        logger.info(
            ...
            f'= {self.ChannelChoose[0].currentText()}, chan2 = {self.ChannelChoose[1].currentText()}')
    ...
    def suggest_model(self, model_name=None):
        ...
        styles_gt = np.load(os.fspath(pathlib.Path.home().joinpath(END_CELLPOSE, 'style_choice.npy')),
