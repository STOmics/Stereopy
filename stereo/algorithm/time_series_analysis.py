...
class TimeSeriesAnalysis(AlgorithmBase)
    ...
    def boxplot_transit_gene(adata, use_col, branch, genes, layer = None, vmax=None, vmin=None):
        """
    ...
    def TVG_marker(adata, use_col, branch, layer=None, p_val_combination='fisher'):
        """
    ...
    def TVG_volcano_plot(adata, use_col, branch):
        """
        ...
        for x in range(N_branch):
            z = 11+x
            ax.scatter(zorder2x[z], zorder2y[z], s=zorder2s[z], zorder=z, color=zorder2color[z],  alpha=0.3, linewidths=0.5, edgecolors='black')
        ...
        for x,y in enumerate(branch):
            ax.scatter([0], [100000000], alpha=0.3, s=300, color=branch_color[x], label=y, linewidths=0.5, edgecolors='black')
    ...
    def fuzzy_C(data, cluster_number, MAX=10000, m=2, Epsilon=1e-7):
        """
        ...
        while (True):
            ...
            U = 1/U
            if epoch % 100 == 0:
                print('epoch {} : time cosumed{:.4f}s, loss:{}'.format(epoch, time.time()-tik, np.max(np.abs(U - U_old))))
                tik = time.time()
    ...
    def fuzzy_C_gene_pattern_cluster(adata, use_col=None, branch=None):
        """
        Use fuzzy C means cluster method to cluster genes based on 1-p_value of celltypes in branch
        :param adata: anndata object to analysis
        :param use_col: the col in obs representing celltype or clustering
        ...
        if ('greater_p' not in adata.varm) or ('less_p' not in adata.varm):
            if use_col==None:
                ...
                TVG_marker(adata, use_col=use_col, branch=branch)
        ...
        adata.varm['fuzzy_C_weight'] = fuzzy_C(adata.varm['feature_p'], 6, Epsilon=1e-7)
        adata.var['fuzzy_C_result'] = np.argmax(adata.varm['fuzzy_C_weight'], axis=1)
    
    
    
    def bezierpath(rs, re, qs, qe, ry, qy, v=True, col='green', alpha=0.2, label='', lw=0, zorder=0):
        """
    ...
    def paga_time_series_plot(adata, use_col = 'celltype', batch_col = 'sample_name', 
        ...
        for x in adata.obs.groupby('tmp'):
            ...
            ax.text(x_tmp, 0-y_tmp, s=x[0].split('|')[0], c='black', alpha =0.8)#, bbox=dict(facecolor='red', alpha=0.1))
            #break
        ...
        for bc in range(len(bc_list)-1):
            for edge in G.edges():
                ...
                if (edge0 in median_center) and (edge1 in median_center) and ((edge[0] in groups)or(edge[1] in groups)):
                    ...
                    if (y1+y2)/2 <threshold_y:
                        ax.annotate('', (x2,y2), (x1,y1), arrowprops=dict(connectionstyle="arc3,rad=0.4", ec = 'black',color='#f9f8e6',arrowstyle = 'simple', alpha=0.5))
                    else:
                        ax.annotate('', (x2,y2), (x1,y1), arrowprops=dict(connectionstyle="arc3,rad=-0.4", ec = 'black',color='#f9f8e6',arrowstyle = 'simple', alpha=0.5))
            ...
            for g in groups:
                for bc in range(len(bc_list)-1):
                    ...
                    if (edge0 in median_center) and (edge1 in median_center):
                        ...
                        p = bezierpath(min_vertex_right[edge0][1], max_vertex_right[edge0][1], \
                                       min_vertex_left[edge1][1], max_vertex_right[edge1][1], \
                                       min_vertex_right[edge0][0], min_vertex_left[edge1][0], True, col=ct2color[g], alpha=link_alpha)
                        ax.add_patch(p)
                
        #显示时期的label
        for x in adata.obs.groupby(batch_col):
            tmp = adata[x[1].index,:].obsm['spatial']
            ax.text(np.mean(tmp[:,0]), 0-np.min(tmp[:,1])+1, s=x[0], c='black', fontsize = 20, ha='center', va='bottom')#, bbox=dict(facecolor='red', alpha=0.1))
