...
2. Fixed a crash issue in `st.io.read_gef` when reading GEF files larger than 30 GB.
3. Fixed missing exon data when saving mid-results to a GEF file using `st.io.write_mid_gef` after analysis.
4. Fixed inconsistent cellid in the output when generating a Loom file via `st.tools.generate_loom`, ensuring they now match those in the source GEF/GEM files.
5. Fixed an error in `st.tools.generate_loom` when processing .tissue.gef files generated by the SAW-ST-V8 pipeline.
