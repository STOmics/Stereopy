...
class MSDataTestCases(unittest.TestCase):
    ...
    def test_length_match_init(self):
        ms_data = MSData(_data_list=[copy.deepcopy(self.obj), copy.deepcopy(self.obj2)], _s_names=['a', 'c'])
        self.assertEqual(len(ms_data._s_names), len(ms_data.data_list))
        self.assertIs(ms_data[0], ms_data['a'])
        self.assertIs(ms_data[1], ms_data['c'])

    def test_length_not_match_init(self):
        ms_data = MSData(_data_list=[copy.deepcopy(self.obj), copy.deepcopy(self.obj2)], _s_names=['a'])
        self.assertEqual(len(ms_data._s_names), len(ms_data.data_list))
        self.assertIs(ms_data[0], ms_data['a'])
    ...
    def test_names(self):
        for idx, data_obj in enumerate(self.ms_data.data_list):
            self.assertIs(self.ms_data[self.ms_data._s_names[idx]], data_obj)
    ...
    def test_rename(self):
        ...
        self.assertNotIn('cc', self.ms_data._s_names)

    def test_names_order(self):
        for idx, name in enumerate(self.ms_data._s_names):
    ...
    def test_s_n(self):
        self.assertEqual(len(self.ms_data), self.ms_data.s_n, len(self.ms_data.data_list))
